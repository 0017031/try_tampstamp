...
int main() {
    ...
    if (auto p = std::find(std::begin(vec1), std::end(vec1), v224); //
        p != std::end(vec1)) {
        cout << "Found " << v224 << " (" << *p << ") at vec[" << p - begin(vec1) << "] !\n";
    }

    if (auto p = std::find_if(std::begin(vec1), std::end(vec1), [](auto &a) { return a == 1; }); //
        p != end(vec1)) {
        cout << "Found "
             << "a==1: " << *p << " at vec[" << p - begin(vec1) << "] !\n";
    }

    if (auto p = std::find_if_not(std::begin(vec1), std::end(vec1), [](auto a) { return a == 1; }); //
        p != end(vec1)) {
        cout << "Found "
    ...
    if (auto p = std::find_end(std::begin(vec2), std::end(vec2), std::begin(pattern), std::end(pattern));
        p != std::end(vec2)) {
